...
import com.macyoo.dbsql.Friend;
import com.macyoo.dbsql.FriendQuery;
import com.macyoo.dbsql.UserQuery;
...
public class HandlerQueue extends Thread {
	...
	public HandlerQueue(MessageBuffer m, PostgresConnectionManager pcm) {
		this.m = m;
		this.pcm = pcm;
		setLogger();
	}
		...
		if( root.get("cmd").equals("login") ) {
			...
			*/
			synchronized( m.replyQueue ) {
				...
				m.replyQueue.offer( new KeyMessage(key, message) );
			}
		}
		else {
	...
	private void DoFriendQuery(SelectionKey key, JSONObject objMessage) {
		...
		try {
			...
			if ( result.isEmpty() ) {
				...
				synchronized( m.replyQueue ) {
					// m.replyQueue.addLast( new KeyMessage(key, message) );
					m.replyQueue.offer( new KeyMessage(key, objReply.toString()) );
				}
			}
