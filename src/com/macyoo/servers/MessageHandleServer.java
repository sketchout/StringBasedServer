...
import com.macyoo.db.connection.DatabasePostgresql;
import com.macyoo.util.LoggerHandle;


public class MessageHandleServer extends StringBasedServer {
	...
	public MessageHandleServer(MessageQueue m, int p) throws UnknownHostException {
		super(p);
		setLogger();
		this.m = m;
	}

	public MessageHandleServer(MessageQueue m,int p,int newBufferSize) throws UnknownHostException {
	...
	protected void processStringMessage(String message, SelectionKey key)
		...
		*/
		synchronized(m.messageQueue) {
			...
			m.messageQueue.offer( new SelectionKeyMessage(key, message) );
			m.readCount++;
